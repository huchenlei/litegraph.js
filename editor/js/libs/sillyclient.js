...
class SillyClient {
    ...
    connect(url, room_name, on_connect, on_message, on_close) {
        room_name = room_name || "";
        var that = this;
        this.url = url;
        if(!url)
    ...
    //Process events 
    onServerEvent(author_id, cmd, data, on_message) {
    ...
    storeData(key, value, on_complete) {
        if(!this.url)
    ...
    //Returns a report with information about clients connected and rooms open
    static getReport(url, on_complete) {
        return new Promise(function(resolve,fail){
        var req = new XMLHttpRequest();
        var protocol = location.protocol + "//";
        if( url.indexOf("wss://") != -1)
    ...
    findRooms(name_str, on_complete) {
        name_str = name_str || "";
        var base_url = this.getBaseURL();
        return new Promise(function(resolve,fail){
            var req = new XMLHttpRequest();
